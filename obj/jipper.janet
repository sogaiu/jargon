...
(def j/loc-grammar
  ~@{:main (sequence (line) (column)
     ...
     #                                         "\n"))
     #                         (line) (column)))
     #      ,|[:whitespace (j/make-attrs ;(slice $& 0 -2)) (last $&)])
     #
     ...
     #                             :form
     #                             (line) (column)))
     #          ,|[:fn (j/make-attrs ;(slice $& 0 2) ;(slice $& -4 -2))
     #             ;(slice $& 2 -4)])
     #
     ...
     #                              $0 $1 ")" :array))))
     #       (line) (column)))
     #   ,|[:array (j/make-attrs ;(slice $& 0 2) ;(slice $& -4 -2))
     #      ;(slice $& 2 -4)])
     #
...
(comment
  ...
  (def j/end-zloc
    (j/right j/start-zloc))

  (j/node j/end-zloc)
  # =>
  :c

  (-> (j/wrap j/start-zloc [] j/end-zloc)
      j/root)
  # =>
  [:a [[:b] :c] :x]

  (try
    (-> (j/wrap j/end-zloc [] j/start-zloc)
        j/root)
    ([e] e))
  # =>
